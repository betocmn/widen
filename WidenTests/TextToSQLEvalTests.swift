...
struct TextToSQLEvalTests {
    ...
    @Test func policyGeneratedWinsClarificationPassesReleaseQualityScoring() async throws {
        let policy = SchemaToolAgentSQLIntentCoveragePolicy.evaluate(
            question: "Tools with the most wins in the last two weeks",
            databaseContext: "",
            ...
            sql: """
                SELECT t.id, COUNT(*) AS wins
                FROM public.preseason_match_evaluation AS e
                JOIN public.preseason_tool AS t ON t.id = e.winner_id
                GROUP BY t.id
                ORDER BY wins DESC
                """
        )
        let clarification = try #require(policy.clarificationQuestion)
        ...
        let evalCase = try #require(
            suite.cases.first { $0.id == "preseason.top-wins-ambiguous" }
        )
        let generator = StaticGenerator(
            result: SQLGenerationResult(
                sql: "",
        ...
        #expect(policy.decision == .mustClarify)
        #expect(clarification == "What should count as one win?")
        #expect(result.status == .passed)
        #expect(result.metrics.clarificationQuality == true)

        let oldWordingResult = await TextToSQLEvalCaseRunner.run(
            evalCase: evalCase,
            schema: makeCommerceSchema(),
            generator: StaticGenerator(
                result: SQLGenerationResult(
                    sql: "",
                    explanation: "Which metric should define wins?",
                    assumptions: [],
                    referencedTables: [],
                    confidence: 0.2,
                    riskLevel: .medium,
        ...
        #expect(oldWordingResult.status == .wrongDecision)
        #expect(oldWordingResult.metrics.clarificationQuality == false)
    }
