...
struct SQLPromptBuilderTests {
    ...
    @Test func promptBundleCapsPinnedTablesAtDetailedCap() {
        let tables = (0..<8).map { index in
            TableInfo(
                schema: "public",
                name: "events_\(index)",
        ...
        let context = SQLGenerationContext(
            ...
            JOIN public.events_2 e2 ON e2.id = e0.id
            JOIN public.events_3 e3 ON e3.id = e0.id
            JOIN public.events_4 e4 ON e4.id = e0.id
            """
        )
        ...
        #expect(bundle.schemaPackage.includedTables.count == 4)
        #expect(!bundle.schemaPackage.includedTables.contains("public.events_4"))
    }
