...
struct OpenRouterSchemaToolSQLAgentTests {
    ...
    @Test func inspectedAmbiguityFallsBackToClarificationWhenToolBudgetIsExhausted() async throws {
        ...
        #expect(result.backendMetadata?.agentDiagnostics?.appSideRejectionReason == .budgetExhausted)
    }
    ...
    @Test func defaultModeConvertsTerminalSQLForUndefinedWinsToClarification() async throws {
        ...
        let scored = TextToSQLEvalScorer.score(
            evalCase: try Self.releaseEvalCase(id: "preseason.top-wins-ambiguous"),
            schema: schema,
            generation: result,
            options: TextToSQLEvalRunOptions(backend: .cloud, model: "test/model"),
            latencyMs: 0
        )
        #expect(scored.status == .passed)
        #expect(scored.metrics.clarificationQuality == true)
    }

    @Test func protectedMetricFallsBackAfterExactlySixSchemaCalls() async throws {
        let schema = Self.makeHealthyAccountSchema()
        let chatTransport = Self.healthyAccountsBudgetExhaustionTransport()
        let agent = makeAgent(
            schema: schema,
            chatTransport: chatTransport,
            configuration: OpenRouterSchemaToolSQLAgentConfiguration(maximumSchemaToolCalls: 6)
        )
        ...
        #expect(result.schemaToolCalls.prefix(6).allSatisfy { $0.errorCode == nil })
    ...
    @Test func definedMetricUsesOneSQLOnlyTurnAfterExactlySixSchemaCalls() async throws {
        let schema = Self.makeHealthyAccountSchema()
        let sql = "SELECT a.id, a.name FROM public.accounts AS a WHERE a.status = 'active' ORDER BY a.id LIMIT 100"
        let chatTransport = Self.healthyAccountsBudgetExhaustionTransport(
            terminalSQL: sql,
            includesTrailingBudgetCall: true
        )
        let agent = makeAgent(
            schema: schema,
            chatTransport: chatTransport,
        ...
        #expect(result.schemaToolCalls.count == 7)
        #expect(result.schemaToolCalls.contains { $0.callID == "trailing-budget-call" } == false)
        #expect(result.schemaToolCalls.last?.errorCode == .sessionBudgetExceeded)
    ...
    @Test func definedMetricCannotInvokeAnEighthSchemaToolAfterBudgetRecovery() async throws {
        let schema = Self.makeHealthyAccountSchema()
        let chatTransport = Self.healthyAccountsBudgetExhaustionTransport(
            violatesTerminalOnlyRecovery: true
        )
        let agent = makeAgent(
            schema: schema,
            chatTransport: chatTransport,
        ...
        do {
            ...
            Issue.record("Expected terminal-only schema budget recovery failure")
            ...
            #expect(failure.schemaToolCalls.prefix(6).allSatisfy { $0.errorCode == nil })
    ...
    @Test func protectedMetricCannotFallbackWithoutSchemaEvidence() async throws {
        let schema = Self.makePreseasonSchema()
        let chatTransport = ScriptedTransport { _, index in
            guard index == 1 else { throw URLError(.badServerResponse) }
            return Self.assistantToolCalls([
                Self.terminalClarification(
                    id: "ungrounded-wins-clarification",
                    question: "What should count as one win?"
                ),
            ])
        }
        let agent = makeAgent(
            ...
            configuration: OpenRouterSchemaToolSQLAgentConfiguration(
                maximumMalformedTerminalCorrections: 0
            )
        )
        ...
        do {
            ...
            Issue.record("Expected ungrounded clarification failure")
        } catch let failure as OpenRouterSchemaToolAgentFailure {
    ...
    @Test func postBudgetSQLRecoveryPreservesWallClockTimeoutAndSevenTraces() async throws {
        let schema = Self.makeHealthyAccountSchema()
        let chatTransport = Self.healthyAccountsBudgetExhaustionTransport(
            hangsDuringTerminalRecovery: true
        )
        let agent = makeAgent(
            schema: schema,
            chatTransport: chatTransport,
        ...
        do {
            ...
            Issue.record("Expected post-budget wall-clock timeout")
        } catch let failure as OpenRouterSchemaToolAgentFailure {
            #expect(failure.category == .wallClockTimeout)
            #expect(chatTransport.requests.count == 4)
            #expect(failure.schemaToolCalls.count == 7)
            #expect(failure.schemaToolCalls.last?.errorCode == .sessionBudgetExceeded)
            #expect(failure.backendMetadata?.agentDiagnostics?.appSideRejectionReason == .timedOut)
        }
    }
    ...
    private static func healthyAccountsBudgetExhaustionTransport(
        terminalSQL: String? = nil,
        violatesTerminalOnlyRecovery: Bool = false,
        includesTrailingBudgetCall: Bool = false,
        hangsDuringTerminalRecovery: Bool = false
    ) -> ScriptedThenHangingTransport {
        ScriptedThenHangingTransport { request, index in
            switch index {
            case 1:
                return Self.assistantToolCalls([
                    Self.toolCall(id: "search-healthy-accounts", name: "search_schema", arguments: [
                        "query": "accounts",
                        "limit": 4,
                    ]),
                    Self.toolCall(id: "search-healthy-memberships", name: "search_schema", arguments: [
                        "query": "account memberships",
                        "limit": 4,
                    ]),
                    Self.toolCall(id: "search-healthy-subscriptions", name: "search_schema", arguments: [
                        "query": "subscriptions",
                        "limit": 4,
                    ]),
                ])
            case 2:
                let accounts = try Self.tableHandle(named: #""public"."accounts""#, in: request)
                let memberships = try Self.tableHandle(
                    named: #""public"."account_memberships""#,
                    in: request
                )
                let subscriptions = try Self.tableHandle(
                    named: #""public"."subscriptions""#,
                    in: request
                )
                return Self.assistantToolCalls([
                    Self.toolCall(id: "describe-healthy-accounts", name: "describe_tables", arguments: [
                        "table_ids": [accounts],
                    ]),
                    Self.toolCall(id: "describe-healthy-memberships", name: "describe_tables", arguments: [
                        "table_ids": [memberships],
                    ]),
                    Self.toolCall(id: "describe-healthy-subscriptions", name: "describe_tables", arguments: [
                        "table_ids": [subscriptions],
                    ]),
                ])
            case 3:
                var calls = [
                    Self.toolCall(id: "over-budget-healthy", name: "search_schema", arguments: [
                        "query": "account health constraints",
                        "limit": 4,
                    ]),
                ]
                if includesTrailingBudgetCall {
                    calls.append(
                        Self.toolCall(id: "trailing-budget-call", name: "search_schema", arguments: [
                            "query": "more account health evidence",
                            "limit": 4,
                        ])
                    )
                }
                return Self.assistantToolCalls(calls)
            case 4:
                let requestText = try Self.requestBodyText(request)
                #expect(requestText.contains("Schema tool budget is exhausted"))
                #expect(try Self.toolDefinitionNames(in: request) == [
                    OpenRouterSchemaToolSQLAgent.terminalToolName,
                ])
                if includesTrailingBudgetCall {
                    let toolMessageIDs = try Self.toolMessageIDs(in: request)
                    #expect(toolMessageIDs.contains("over-budget-healthy"))
                    #expect(toolMessageIDs.contains("trailing-budget-call"))
                }
                if hangsDuringTerminalRecovery { return nil }
                if violatesTerminalOnlyRecovery {
                    return Self.assistantToolCalls([
                        Self.toolCall(id: "eighth-schema-call", name: "search_schema", arguments: [
                            "query": "more account health evidence",
                            "limit": 4,
                        ]),
                    ])
                }
                guard let terminalSQL else { throw URLError(.badServerResponse) }
                return Self.assistantToolCalls([
                    Self.terminalSQL(id: "terminal-after-healthy-budget", sql: terminalSQL),
                ])
            default:
                throw URLError(.badServerResponse)
            }
    ...
    private static func toolDefinitionNames(in request: URLRequest) throws -> [String] {
        let body = try requestBody(request)
    ...
    private static func releaseEvalCase(id: String) throws -> TextToSQLEvalCase {
        let root = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let data = try Data(
            contentsOf: root.appendingPathComponent("Evals/suites/text-to-sql-v1.json")
        )
        ...
        DatabaseSchema(
            ...
            tables: [
                TableInfo(
                    ...
                    type: .baseTable,
                    columns: [
                        column("accounts", "id", type: "integer", ordinal: 1),
                        column("accounts", "name", type: "text", ordinal: 2),
                        column("accounts", "status", type: "text", ordinal: 3),
                    ],
                    keyConstraints: [
                        SchemaKeyConstraintInfo(
                ...
                TableInfo(
                    ...
                    type: .baseTable,
                    columns: [
                        column("account_memberships", "id", type: "integer", ordinal: 1),
                        column("account_memberships", "account_id", type: "integer", ordinal: 2),
                        column("account_memberships", "user_id", type: "integer", ordinal: 3),
                    ]
                ),
                TableInfo(
                    schema: "public",
                    ...
                    type: .baseTable,
                    columns: [
                        column("subscriptions", "id", type: "integer", ordinal: 1),
                        column("subscriptions", "account_id", type: "integer", ordinal: 2),
                        column("subscriptions", "status", type: "text", ordinal: 3),
                    ]
                ),
            ],
            foreignKeyConstraints: [
                SchemaForeignKeyConstraintInfo(
                    constraintName: "account_memberships_account_id_fkey",
                    sourceSchema: "public",
                    sourceTable: "account_memberships",
                    targetSchema: "public",
                ...
                SchemaForeignKeyConstraintInfo(
                    constraintName: "subscriptions_account_id_fkey",
                    sourceSchema: "public",
