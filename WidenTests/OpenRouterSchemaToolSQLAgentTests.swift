...
struct OpenRouterSchemaToolSQLAgentTests {
    ...
    @Test func schemaEvidenceDecodesOlderPayloadWithoutQuestionRelevantSearchIDs() throws {
        let data = Data(#"{"searched":true,"describedTableIDs":["public.organization"]}"#.utf8)
        ...
        #expect(evidence.searched)
        #expect(evidence.questionRelevantSearchedTableIDs.isEmpty)
        #expect(evidence.describedTableIDs == ["public.organization"])
    }
    ...
    private actor BlockingFingerprintSequence {
        private let initial: String
        private let blockAtCallCount: Int
        private var callCount = 0
        private var didBlock = false
        private var blockedWaiters: [CheckedContinuation<Void, Never>] = []
        private var releaseContinuation: CheckedContinuation<Void, Never>?

        init(initial: String, blockAtCallCount: Int) {
            self.initial = initial
            self.blockAtCallCount = blockAtCallCount
        }

        func current() async -> String {
            callCount += 1
            guard callCount == blockAtCallCount else { return initial }
            didBlock = true
    ...
    private actor DelayedFingerprintSequence {
        private let initial: String
        private let delayAtCallCount: Int
        private let delayNanoseconds: UInt64
        private var callCount = 0

        init(initial: String, delayAtCallCount: Int, delayNanoseconds: UInt64) {
            self.initial = initial
            self.delayAtCallCount = delayAtCallCount
            self.delayNanoseconds = delayNanoseconds
        }

        func current() async throws -> String {
            callCount += 1
            if callCount == delayAtCallCount {
    ...
    @Test func protectedMetricClarifiesAfterSixSuccessfulCallsAndRejectedSeventh() async throws {
        let schema = Self.makeHealthyAccountSchema()
        let chatTransport = Self.protectedMetricBudgetTransport()
        let agent = makeAgent(
            schema: schema,
            chatTransport: chatTransport,
        ...
        #expect(result.clarificationQuestion == "Which metric should define healthy accounts?")
        #expect(chatTransport.requests.count == 3)
        #expect(result.schemaToolCalls.count == 7)
        #expect(result.schemaToolCalls.prefix(6).allSatisfy {
            $0.outcome == .success && $0.errorCode == nil
        })
        #expect(result.schemaToolCalls.last?.outcome == .error)
        #expect(result.schemaToolCalls.last?.toolName == "inspect_column_constraints")
        #expect(result.schemaToolCalls.last?.errorCode == .sessionBudgetExceeded)
        #expect(!result.schemaToolCalls.contains { $0.callID == "trailing-eighth-call" })
        #expect(result.backendMetadata?.agentSchemaToolCallCount == 7)
        #expect(result.backendMetadata?.agentTerminalOutcome == "clarify_fallback")
        let diagnostics = try #require(result.backendMetadata?.agentDiagnostics)
        #expect(!diagnostics.terminalToolSeen)
        #expect(diagnostics.terminalAction == "clarify")
        #expect(diagnostics.appSideRejectionReason == .budgetExhausted)
        #expect(diagnostics.terminalValidationFailureReason == "budgetExhaustedWithProtectedAmbiguity")
        #expect(diagnostics.clarificationPolicyDecision == "acceptableAmbiguity")
        #expect(diagnostics.sqlIntentCoverageDecision == "mustClarify")
        #expect(diagnostics.redundantDuplicateToolCallCount == 0)
        #expect(diagnostics.redundantZeroResultSearchCount == 0)
        #expect(diagnostics.redundantJoinPathCallCount == 0)
        #expect(
            diagnostics.schemaEvidence.questionRelevantSearchedTableIDs.contains(
                "public.organization"
            )
        )
        #expect(
    ...
    @Test func protectedMetricBudgetFallbackRejectsContextDefinedMetric() async throws {
        let schema = Self.makeHealthyAccountSchema()
        let chatTransport = Self.protectedMetricBudgetTransport()
        let agent = makeAgent(
            schema: schema,
        ...
        do {
            ...
            Issue.record("Expected the ordinary schema-tool budget failure")
        } catch let failure as OpenRouterSchemaToolAgentFailure {
            #expect(failure.category == .schemaToolCallBudgetExhausted)
            #expect(chatTransport.requests.count == 3)
            #expect(failure.schemaToolCalls.count == 7)
            #expect(failure.schemaToolCalls.prefix(6).allSatisfy {
                $0.outcome == .success && $0.errorCode == nil
            })
            #expect(failure.schemaToolCalls.last?.toolName == "inspect_column_constraints")
            #expect(failure.schemaToolCalls.last?.errorCode == .sessionBudgetExceeded)
            #expect(!failure.schemaToolCalls.contains { $0.callID == "trailing-eighth-call" })
            #expect(failure.backendMetadata?.agentTerminalOutcome == nil)
            let diagnostics = try #require(failure.backendMetadata?.agentDiagnostics)
            #expect(!diagnostics.terminalToolSeen)
            #expect(diagnostics.terminalAction == nil)
            #expect(diagnostics.appSideRejectionReason == .budgetExhausted)
            #expect(
                diagnostics.terminalValidationFailureReason
                    == "budgetExhaustedAfterSufficientEvidence"
            )
            #expect(diagnostics.clarificationPolicyDecision == "shouldAnswerWithSQL")
            #expect(diagnostics.evidenceSufficientForSQL)
            #expect(diagnostics.databaseContextFactsUsed.contains("metric"))
            #expect(
                diagnostics.schemaEvidence.questionRelevantSearchedTableIDs.contains(
                    "public.organization"
                )
    ...
    @Test func protectedMetricBudgetFallbackRejectsConfirmedMetricBinding() async throws {
        let schema = Self.makeHealthyAccountSchema()
        let chatTransport = Self.protectedMetricBudgetTransport()
        let agent = makeAgent(
            schema: schema,
            ...
            _ = try await agent.generateSQL(
                ...
                context: SQLGenerationContext(
                    confirmedSemanticBindings: [
                        "healthy: accounts with active status"
                    ]
                ),
                config: SQLGenerationConfig()
            )
    ...
    @Test func protectedMetricBudgetFallbackRequiresStandaloneInitialContext() async throws {
        let schema = Self.makeHealthyAccountSchema()
        let chatTransport = Self.protectedMetricBudgetTransport()
        let agent = makeAgent(
            schema: schema,
        ...
        do {
            _ = try await agent.generateSQL(
                ...
                schema: schema,
                context: SQLGenerationContext(
                    recentQuestions: ["Show all organizations"],
                    originalQuestion: "Show all organizations",
                    conversationMessages: [
                        SQLConversationMessage(role: .user, text: "Show all organizations")
                    ],
                    currentSQL: "SELECT id FROM public.organization"
                ),
                config: SQLGenerationConfig()
            )
            Issue.record("Expected the ordinary schema-tool budget failure")
    ...
    @Test(arguments: [
        ...
        (
            searchPrefix: "healthy ",
            question: "Which accounts are healthy?"
        ),
    ])
    func protectedMetricBudgetFallbackRequiresSubjectRelevantSearches(
        searchPrefix: String,
        question: String
    ) async throws {
        let schema = Self.makeHealthyAccountSchema()
        ...
        do {
            ...
            #expect(diagnostics.clarificationPolicyDecision.isEmpty)
            #expect(diagnostics.sqlIntentCoverageDecision.isEmpty)
            #expect(!diagnostics.evidenceSufficientForSQL)
            #expect(diagnostics.schemaEvidence.questionRelevantSearchedTableIDs.isEmpty)
            #expect(diagnostics.schemaEvidence.searched)
            #expect(diagnostics.schemaEvidence.describedTableIDs.count == 3)
            #expect(!diagnostics.schemaEvidence.exposedColumnIDs.isEmpty)
            #expect(!diagnostics.schemaEvidence.exposedForeignKeyPathIDs.isEmpty)
        }
    }

    @Test func staleSnapshotPrecedesProtectedFallbackAfterRejectedSeventhCall() async throws {
        let schema = Self.makeHealthyAccountSchema()
        ...
        let sequence = FingerprintSequence(initial: fingerprint, validCallCount: 10)
        let chatTransport = Self.protectedMetricBudgetTransport()
        let agent = makeAgent(
            schema: schema,
            chatTransport: chatTransport,
        ...
        do {
            ...
            Issue.record("Expected stale schema snapshot at the protected budget boundary")
        } catch let failure as OpenRouterSchemaToolAgentFailure {
            ...
            #expect(failure.schemaToolCalls.last?.errorCode == .sessionBudgetExceeded)
            #expect(!failure.schemaToolCalls.contains { $0.callID == "trailing-eighth-call" })
            #expect(failure.backendMetadata?.agentDiagnostics?.appSideRejectionReason == nil)
        }
    }

    @Test func cancellationPrecedesProtectedFallbackAfterRejectedSeventhCall() async throws {
        let schema = Self.makeHealthyAccountSchema()
        let snapshot = SchemaSearchSnapshot(
            connectionID: Self.connectionID,
            selectedSchemas: ["public"],
        ...
        let fingerprint = try SchemaSearchIndexStore.cacheKey(for: snapshot).schemaFingerprint
        let sequence = BlockingFingerprintSequence(initial: fingerprint, blockAtCallCount: 11)
        let chatTransport = Self.protectedMetricBudgetTransport()
        let agent = makeAgent(
            schema: schema,
            chatTransport: chatTransport,
            ...
            currentSchemaFingerprint: { await sequence.current() }
        )
        let generation = Task {
            try await agent.generateSQL(
                question: "Which accounts are healthy?",
                schema: schema,
                context: SQLGenerationContext(),
                config: SQLGenerationConfig()
        ...
        await sequence.waitUntilBlocked()
        generation.cancel()
        await sequence.release()
        do {
            _ = try await generation.value
            Issue.record("Expected cancellation at the protected budget boundary")
        } catch is CancellationError {
            #expect(chatTransport.requests.count == 3)
        }
    }

    @Test func timeoutPrecedesProtectedFallbackAfterRejectedSeventhCall() async throws {
        let schema = Self.makeHealthyAccountSchema()
        let snapshot = SchemaSearchSnapshot(
            connectionID: Self.connectionID,
            selectedSchemas: ["public"],
        ...
        let fingerprint = try SchemaSearchIndexStore.cacheKey(for: snapshot).schemaFingerprint
        let sequence = DelayedFingerprintSequence(
            initial: fingerprint,
            delayAtCallCount: 11,
        ...
        let agent = makeAgent(
            ...
            configuration: OpenRouterSchemaToolSQLAgentConfiguration(
                maximumSchemaToolCalls: 6,
                wallClockTimeoutSeconds: 0.2
            ),
            currentSchemaFingerprint: { try await sequence.current() }
        )
        ...
        do {
            ...
            Issue.record("Expected timeout at the protected budget boundary")
        } catch let failure as OpenRouterSchemaToolAgentFailure {
            ...
            #expect(!failure.schemaToolCalls.contains { $0.callID == "trailing-eighth-call" })
            #expect(
                failure.backendMetadata?.agentDiagnostics?.appSideRejectionReason == .timedOut
            )
        }
    }
    ...
    private static func protectedMetricBudgetTransport(
        searchPrefix: String = "healthy accounts "
    ) -> ScriptedTransport {
        ScriptedTransport { request, index in
            switch index {
            case 1:
                return Self.assistantToolCalls([
                    Self.toolCall(id: "search-healthy-organizations", name: "search_schema", arguments: [
                        "query": "\(searchPrefix)organization",
                        "limit": 4,
                    ]),
                    Self.toolCall(id: "search-organization-memberships", name: "search_schema", arguments: [
                        "query": "\(searchPrefix)organization membership",
                        "limit": 4,
                    ]),
                    Self.toolCall(id: "search-subscriptions", name: "search_schema", arguments: [
                        "query": "\(searchPrefix)subscription",
                        "limit": 4,
                    ]),
                ])
            case 2:
                let organizations = try Self.tableHandle(
                    named: #""public"."organization""#,
                    in: request
                )
                let memberships = try Self.tableHandle(
                    named: #""public"."organization_membership""#,
                    in: request
                )
                let subscriptions = try Self.tableHandle(
                    named: #""public"."subscription""#,
                    in: request
                )
                return Self.assistantToolCalls([
                    Self.toolCall(id: "describe-healthy-organizations", name: "describe_tables", arguments: [
                        "table_ids": [organizations],
                    ]),
                    Self.toolCall(id: "describe-organization-memberships", name: "describe_tables", arguments: [
                        "table_ids": [memberships],
                    ]),
                    Self.toolCall(id: "describe-subscriptions", name: "describe_tables", arguments: [
                        "table_ids": [subscriptions],
                    ]),
                ])
            case 3:
                let handles = try Self.describedColumnHandles(
                    tableSQLName: #""public"."subscription""#,
                    columnSQLName: #""public"."subscription"."status""#,
                    in: request
                )
                return Self.assistantToolCalls([
                    Self.toolCall(
                        id: "rejected-seventh-call",
                        name: "inspect_column_constraints",
                        arguments: [
                            "table_id": handles.table,
                            "column_id": handles.column,
                        ]
                    ),
                    Self.toolCall(id: "trailing-eighth-call", name: "search_schema", arguments: [
                        "query": "more account health evidence",
                        "limit": 4,
                    ]),
                ])
            default:
    ...
    private static func describedColumnHandles(
        tableSQLName: String,
        columnSQLName: String,
        in request: URLRequest
    ) throws -> (table: String, column: String) {
        for result in try toolResults(in: request) {
            guard let tables = result.payload?["tables"]?.arrayValue else { continue }
            for table in tables where table["sql_name"]?.stringValue == tableSQLName {
                guard let tableHandle = table["table_id"]?.stringValue,
                    let columns = table["columns"]?.arrayValue
                else {
                    continue
                }
                for column in columns where column["sql_name"]?.stringValue == columnSQLName {
                    if let columnHandle = column["column_id"]?.stringValue {
                        return (tableHandle, columnHandle)
                    }
                }
            }
        }
    ...
    private static func toolResults(in request: URLRequest) throws -> [SchemaToolResult] {
        ...
        return messages.compactMap { message -> SchemaToolResult? in
    ...
    private static func makeHealthyAccountSchema() -> DatabaseSchema {
        DatabaseSchema(
            schemas: [SchemaInfo(name: "public")],
            tables: [
                TableInfo(
                    ...
                    type: .baseTable,
                    columns: [
                        column("organization", "id", type: "integer", ordinal: 1),
                        column("organization", "name", type: "text", ordinal: 2),
                        column("organization", "status", type: "text", ordinal: 3),
                    ]
                ),
                TableInfo(
                    schema: "public",
                    ...
                    type: .baseTable,
                    columns: [
                        column("organization_membership", "id", type: "integer", ordinal: 1),
                        column("organization_membership", "organization_id", type: "integer", ordinal: 2),
                        column("organization_membership", "user_id", type: "integer", ordinal: 3),
                    ]
                ),
                TableInfo(
                    schema: "public",
                    ...
                    type: .baseTable,
                    columns: [
                        column("subscription", "id", type: "integer", ordinal: 1),
                        column("subscription", "organization_id", type: "integer", ordinal: 2),
                        column("subscription", "status", type: "text", ordinal: 3),
                    ]
                ),
            ],
            foreignKeyConstraints: [
                ...
                SchemaForeignKeyConstraintInfo(
                    constraintName: "subscription_organization_id_fkey",
                    sourceSchema: "public",
