...
enum SchemaRetrievalEvalReporter {
    ...
    private static func summarySection(_ summary: SchemaRetrievalSummary) -> [String] {
        [
            ...
            "| Missing required column matches | \(summary.missingRequiredColumnMatchCount) |",
            ...
            "| Index build duration max | \(summary.indexBuildDurationMs.map { "\($0) ms" } ?? "-") |",
            "| Index serialized size max | \(summary.indexSerializedSizeBytes.map { "\($0) bytes" } ?? "-") |",
